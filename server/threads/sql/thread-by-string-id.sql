SELECT
    ...
    (SELECT slug FROM boards WHERE boards.id = threads.parent_board )as board_slug,
    ...
    COALESCE(COUNT(parent_post_id = first_post.string_id)::int, 0) as thread_direct_threads_amount,
...
FROM threads
LEFT JOIN (SELECT id FROM users WHERE users.firebase_id = ${firebase_id} LIMIT 1) logged_in_user
    ON ${firebase_id} IS NOT NULL
LEFT JOIN thread_notification_dismissals
    ON logged_in_user.id IS NOT NULL 
        AND thread_notification_dismissals.thread_string_id = ${thread_string_id}
        AND thread_notification_dismissals.user_id = logged_in_user.id
LEFT JOIN LATERAL (
    SELECT
        ...
        comments,
        COALESCE(total_comments_amount, 0) as total_comments_amount,
    ...
    LEFT JOIN LATERAL (
          SELECT 
      parent_post,
            json_agg(comments_agg) as comments,
            COALESCE(COUNT(comments_agg), 0) as total_comments_amount,
            COALESCE(COUNT(CASE WHEN is_new THEN 1 END), 0) as new_comments_amount
          FROM (
              SELECT
              ...
              WHERE comment_details.parent_thread_id = threads.id AND comment_details.parent_post_id = post_details.post_id
              ORDER BY created ASC) AS comments_agg
              GROUP BY parent_post
      ) thread_comments ON thread_comments.parent_post = post_details.post_string_id
    WHERE post_details.parent_thread_id = threads.id
    ORDER BY post_details.created ASC
) thread_posts ON TRUE
LEFT JOIN LATERAL (
  SELECT string_id FROM posts WHERE posts.parent_thread = threads.id AND posts.parent_post IS NULL
) first_post ON TRUE
